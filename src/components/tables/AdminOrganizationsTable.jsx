...
const AdminOrganizationsTable = ({ data, setCurrentPageFetch }) => {
    const dispatch = useDispatch()

    const { modals, setModals } = useStateContext()

    const { total_organization_pages } = useSelector(state => state.admin)
    ...
    useEffect(() => {
        ...
        if (total_organization_pages) {
            for (var i = 0; i < total_organization_pages; i++) {
                pages.push(count)
                count++
            }
                                        ...
                                        <td className=" flex text-2xl space-x-4 text-gray-900 font-semibold py-4 whitespace-nowrap">
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setModals({ ...modals, showAdminViewOrganizationModal: !modals.showAdminViewOrganizationModal })
                                                }}
                                            >
                                                View
                                            </button>
